...
class Conv2D():
    ...
    def __init__(self, kernels_num, kernel_shape, input_shape = None, activation = None, padding = (0, 0), stride = (1, 1), dilation = (1, 1), use_bias = True, data_type = np.float32):
    ...
    def build(self):
        ...
        self.w = np.random.uniform(-stdv, stdv, (self.kernels_num, self.channels_num, self.kernel_height, self.kernel_width)).astype(self.data_type)
        # self.w = np.random.normal(0, 1, (self.kernels_num, self.channels_num, self.kernel_height, self.kernel_width)) * np.sqrt(2. / self.channels_num * self.prepared_input_height * self.prepared_input_width)
        # self.w = self.w.astype(self.data_type)
        self.b = np.zeros(self.kernels_num).astype(self.data_type)
    ...
    def remove_padding(layer, padding):
        unpadded_layer = layer[:, :, padding[0]:-padding[1], padding[1]:-padding[0]]
